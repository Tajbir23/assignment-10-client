...
const DeleteModal = () => {
  const { deleteItem, setDeleteModal, render, setRender } =
  ...
  const handleDelete = (e) => {
    ...
    fetch(
      `https://art-craft-server-three.vercel.app/delete/${deleteItem._id}`,
      {
        method: "DELETE",
        headers: {
    ...
    <div className="flex flex-col max-w-md gap-2 p-6 rounded-md shadow-md dark:bg-gray-50 dark:text-gray-800">
      <h2 className="flex items-center gap-2 text-xl font-semibold leading-tight tracking-wide">
        <svg
          ...
          viewBox="0 0 512 512"
          className="w-6 h-6 fill-current shrink-0 dark:text-violet-600"
        >
      ...
      <p className="flex-1 dark:text-gray-600">{deleteItem?.item_name}</p>
      <div className="flex flex-col justify-end gap-3 mt-6 sm:flex-row">
        <button onClick={handleClose} className="px-6 py-2 rounded-sm">
          No
        </button>
        <button
